...
static vector<boost::shared_ptr<Solver<float> > > solvers_;
static vector<boost::shared_ptr<Net<float> > > nets_;
...
// Convert a vector of boost::shared_ptr in C++ to handle struct vector
template <typename T>
static mxArray* ptr_vec_to_handle_vec(const vector<boost::shared_ptr<T> >& ptr_vec) {
...
static void get_solver(MEX_ARGS) {
	...
	boost::shared_ptr<Solver<float> > solver(new caffe::SGDSolver<float>(solver_file));
...
static void get_net(MEX_ARGS) {
	...
	boost::shared_ptr<Net<float> > net(new caffe::Net<float>(model_file, phase));
	nets_.push_back(net);
