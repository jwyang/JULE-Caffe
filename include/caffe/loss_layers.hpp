...
/**
* @brief Computes the graph linkage loss, consists of intra loss and inter loss.
* L = L_intra(\theta|I,y) + L_extra(\theta|I,y)
* 
* added by Jianwei Yang @10/03/2015
*
*/
template <typename Dtype>
class GraphLinkageLossLayer : public LossLayer<Dtype> {
public:
	explicit GraphLinkageLossLayer(const LayerParameter& param)
		: LossLayer<Dtype>(param), diff_() {}
	virtual void LayerSetUp(const vector<Blob<Dtype>*>& bottom,
		const vector<Blob<Dtype>*>& top);

	virtual inline int ExactNumBottomBlobs() const { return 2; }
	virtual inline const char* type() const { return "GraphLinkageLoss"; }
	/**
	* Unlike most loss layers, in the ContrastiveLossLayer we can backpropagate
	* to the first two inputs.
	*/
...
protected:
	...
	/**
	* @brief Computes the Graphlinkage error gradient w.r.t. the inputs.
	*
	*/
	virtual void Backward_cpu(const vector<Blob<Dtype>*>& top,
		const vector<bool>& propagate_down, const vector<Blob<Dtype>*>& bottom);
	...
	int num_items_;
	Dtype sigma_;       // stddev for computing affinity matrix
	Dtype margin_;
	Blob<Dtype> diff_;  // cached for backward pass
	Blob<Dtype> diff_sq_;  // tmp storage for gpu forward pass
	...
	Blob<Dtype> affinity_sample_;  // affinity matrix among samples
	Blob<Dtype> affinity_intra_;   // intra affinity for each category
	Blob<Dtype> affinity_extra_;   // extra affinity matrix among categories
	Blob<Dtype> summer_vec_;  // tmp storage for gpu forward pass

	vector<int> labels_idx_;
	vector<vector<int> > label_indice_;

	vector<pair<int, int> > rec_pos_;
	vector<pair<int, int> > rec_neg_;

	int num_intra_valid_;
	int num_extra_valid_;
