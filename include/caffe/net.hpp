...
class Net {
 public:
  ...
  bool SetDataLabels(int* labels, int idx_start, int idx_end); // @set data labels.  
  bool SetDataPos(int pos); // @seek to specified data position 
  int* GetDataGTLabels(); // @get groundtruth data labels.  
  int  GetDataSize();       // @get data size for data layer
  int  GetDataPos();        // @get data position for data layer
  int  GetMiniBatchSize();  // @get mini batch size for data layer
  const shared_ptr<Layer<Dtype> > GetLayer(const int& id) const;
