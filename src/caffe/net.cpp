...
// set data labels in [idx_start, idx_end]
template <typename Dtype>
bool Net<Dtype>::SetDataLabels(int* labels, int idx_start, int idx_end) {
	// shared_ptr<DataLayer<Dtype> > layer =
		// static_cast<shared_ptr<BaseDataLayer<Dtype> > >(layers_[layer_names_index_.find("data")->second]);
	return layers_[layer_names_index_.find("data")->second]->set_data_labels(labels, idx_start, idx_end);
}

// added by Jianwei Yang
// set data labels in [idx_start, idx_end]
template <typename Dtype>
bool Net<Dtype>::SetDataPos(int pos) {
	return layers_[layer_names_index_.find("data")->second]->set_data_pos(pos);
}

// added by Jianwei Yang
// get current data position in data layer
template <typename Dtype>
int Net<Dtype>::GetDataPos() {
	return layers_[layer_names_index_.find("data")->second]->get_data_pos();
}

// added by Jianwei Yang
// get data size in data layer
template <typename Dtype>
int Net<Dtype>::GetDataSize() {
	return layers_[layer_names_index_.find("data")->second]->get_data_size();
}

// added by Jianwei Yang
// get groundtruth data labels in data layer
template <typename Dtype>
int* Net<Dtype>::GetDataGTLabels() {
	return layers_[layer_names_index_.find("data")->second]->get_data_gtlabels();
}

// added by Jianwei Yang
// get batch size  for data layer
template <typename Dtype>
int Net<Dtype>::GetMiniBatchSize() {
	return layers_[layer_names_index_.find("data")->second]->get_data_minibatch_size();
}

// added by Jianwei Yang
// get net layer
template <typename Dtype>
const shared_ptr<Layer<Dtype> > Net<Dtype>::GetLayer(const int& id) const {
	return layers_[id];
}
